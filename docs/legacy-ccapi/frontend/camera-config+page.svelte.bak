...
<main class="min-h-screen bg-neutral-950 text-white p-8 flex flex-col gap-6 max-w-md mx-auto">
  ...
  <section class="flex flex-col gap-3">
    <h2 class="text-lg font-medium text-neutral-200 border-b border-neutral-800 pb-2">📷 Kamera (CCAPI)</h2>
    ...
    <div class="flex flex-col gap-2">
      ...
      >
        {cameraStore.status === "connecting" ? "Menghubungkan..." : "Hubungkan Kamera"}
      </button>
    ...
    {#if cameraStore.status === "connected"}
      <div class="bg-neutral-900 border border-neutral-800 p-4 rounded-lg flex flex-col gap-3 mt-2">
        ...
        <p class="font-mono text-sm text-green-400">
          {cameraStore.device?.productname || "Canon Camera"} {cameraStore.device?.serialnumber ? `(${cameraStore.device?.serialnumber})` : ""}
        </p>
