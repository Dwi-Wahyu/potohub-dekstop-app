<script lang="ts">
  ...
  async function handleCapture() {
    ...
    if (autoPrint && printerStore.selectedPrinter) {
      // Direct print simulation buffer / demo frame
      const dummyBuffer = new Uint8Array([255, 216, 255, 224]); // JPEG magic header byte demo
      await printerStore.printFromBuffer(dummyBuffer, {
        copies: printCopies,
  ...
  async function handleManualPrint() {
    if (!printerStore.selectedPrinter) return;
    const dummyBuffer = new Uint8Array([255, 216, 255, 224]);
    await printerStore.printFromBuffer(dummyBuffer, {
      copies: printCopies,
      paper_size: paperSize,
      quality: "standard",
