...
- **Penggunaan:** Digunakan sebagai acuan visual utama untuk merancang antarmuka neumorfik pada layar _Welcome_ dan _Kode Aktivasi_ (`src/routes/onboarding/+page.svelte`).
...
- Membuat wrapper TypeScript untuk mengelola status aktivasi booth di SQLite (`sqlite:app.db`).
...
1. **`src/routes/onboarding/+page.svelte`** (**BARU**):
   - Wizard alur _Welcome_ dan _Kode Aktivasi_ dengan desain neumorfik sesuai referensi `Perangkat.tsx`.
