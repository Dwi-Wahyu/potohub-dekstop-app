<script lang="ts">
  ...
  onMount(async () => {
    ...
    try {
      ...
      console.error('Failed to fetch template in V3Download:', err);
    }
    ...
    if (networkStatus.isOnline) {
      ...
      try {
        ...
        console.error('Failed to create session online in V3Download, using local code:', err);
    ...
    try {
      ...
      console.error('Failed to save assets in V3Download:', err);
    } finally {
      isSaving = false;
    }
  });
        ...
        {#if !sent}
          <div class="p-5 flex flex-col gap-3 font-mono">
            ...
            {:else if softfileQueued}
              <div class="text-[10px] text-amber-400 font-bold mb-1 font-mono">Menunggu koneksi pulih untuk mengirim softfile…</div>
            {/if}
            ...
            {#if emailEnabled}
              <div class="flex flex-col gap-1.5">
                <div class="flex items-center justify-between text-[10px] text-white/70 font-bold">
                  <span>EMAIL</span>
                  {#if emailSent}
                    <span class="text-[#FFC107] font-bold">
